...
class OrderLine(Base):
    ...
    هویتِ خط دو شاخه دارد (`repo_import.py`):

    * خطِ **فاکتوردار**: `line_uid_for_order(business, دوره, فاکتورِ نرمال‌شده,
      کالای نرمال‌شده, نوع, ترتیب)` — مبلغ عمداً در کلید نیست تا صادراتِ دوباره
      با مبلغِ اصلاح‌شده همان خط را به‌روز کند، نه اینکه دوبار بشمارد.
    * خطِ **بی‌فاکتور**: `line_uid = sha256(business|dataset|sheet|source_row)`
      — فقط با همان فایل و همان ردیف یکی است.
    """
