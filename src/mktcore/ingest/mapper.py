...
# نوع سند: متنی با تنوع کم؛ «نحوه/شماره/مبلغ/روش» عمداً بلاک تا با ستون‌های
# دیگر (مثل «نحوه برگشت از فروش» یا «شماره سند») قاطی نشود
DOC_TYPE_KW = [("نوع سند", 1.0), ("نوع فاکتور", 0.9), ("نوع تراکنش", 0.85),
               ("doc type", 0.9), ("document type", 0.9)]
DOC_TYPE_BLOCK = ["نحوه", "شماره", "تاریخ", "مبلغ", "روش"]
# مبلغ ناخالص: عمداً محدود؛ «جمع کل/مبلغ کل» اضافه نمی‌شود چون کلیدواژه‌ی
# قوی REVENUE است و فایل تک‌ستونه باید همان REVENUE بماند
GROSS_KW = [("مبلغ ناخالص", 1.0), ("جمع ناخالص", 0.95),
            ("مبلغ قبل از تخفیف", 0.95), ("قیمت کل", 0.85), ("gross amount", 0.95)]
...
class SchemaMapper:
    ...
    def _score(self, role: ColumnRole, p: ColumnProfile) -> tuple[float, str]:
        ...
        if role == ColumnRole.DOC_TYPE:
            if _has_block(cn, DOC_TYPE_BLOCK):
                return 0.0, ""
            w, kw = _name_match(cn, DOC_TYPE_KW)
            # نوع سند: متنی و کم‌تنوع (فروش/برگشت/ابطال…)
            if w == 0 or p.text_ratio < 0.6 or p.n_unique > 15:
                return 0.0, ""
            return w * 0.85, f"نام ستون شامل «{kw}» و مقادیر کم‌تنوع متنی است"

        if role == ColumnRole.GROSS_AMOUNT:
            w, kw = _name_match(cn, GROSS_KW)
            if w == 0 or p.numeric_ratio < 0.70:
                return 0.0, ""
            return w * (0.5 + 0.45 * p.numeric_ratio), f"نام ستون شامل «{kw}» و {npct}٪ عددی است"
