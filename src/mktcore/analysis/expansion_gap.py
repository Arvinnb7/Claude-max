...
شکاف، درآمدِ قطعی نیست؛ یک **تخمین رتبه‌بندی‌شده از پتانسیل** است. و بر پایه‌ی
**درآمد** است نه سود ناخالص — برخلاف §۱۷.۳ سند که سود می‌خواهد.

⚠️ این دیگر محدودیتِ داده نیست: بهای تمام‌شده از فاز سود ناخالص به بعد در دفتر
کل موجود است (`order_lines.gross_profit_rial`). این ماژول عمداً هنوز درآمدی
مانده تا رتبه‌بندیِ صندوق فرصت‌ها بدون سنجش تغییر نکند؛ نسخه‌ی سودمحورش وقتی
می‌آید که بشود با همان دروازه‌ی «مدعی باید قهرمان را ببرد» سنجیدش.
"""
