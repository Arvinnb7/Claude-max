...
EXTRA_GUARDED_ROUTES: dict[str, str] = {
    ...
    "GET /api/v1/contact-suppressions": (
        "دفترِ «تماس نگیر»: شماره (ماسک‌شده) و نامِ مشتری‌هایی که انصراف داده‌اند — "
        "فهرستِ افرادِ مشخص، پس داده‌ی شخصی است."
    ),
    "GET /api/v1/feature-basis-diff": (
        "کلِ دفتر کل را می‌خواند و پرونده‌ی ۳۶۰ را دوباره می‌سازد (هزینه‌بر) و "
        "اعدادِ خریدِ تک‌تکِ مشتری‌ها را کنارِ شناسه‌شان برمی‌گرداند."
    ),
    "GET /api/v1/quarantine": (
        "ردیفِ خامِ فایلِ فروش را همان‌طور که بود برمی‌گرداند — با ستونِ موبایل "
        "و نامِ مشتری. شماره‌ها ماسک می‌شوند، ولی بقیه‌ی ردیف هم داده‌ی شخصی است."
    ),
}
