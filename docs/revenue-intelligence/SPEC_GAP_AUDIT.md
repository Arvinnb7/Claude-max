...
| data-quality UI/API (§۸.۵ — ۹ بُعد) | ✅ **بسته شد (S5, F3)** | هر نُه بُعد؛ بُعدی که مبنایش نیست `None` («سنجیده نشد») می‌گیرد، نه صفر؛ از F3 **به‌ازای هر بارگذاری** هم ثبت و نمایش داده می‌شود. **محدودیتِ شناخته‌شده‌ی داشبورد**: `date_range_consistency`ِ `GET /data-quality` بازه را از خودِ خطوطِ آخرین دسته می‌سازد و همیشه ۱٫۰ می‌دهد (همان‌گویی)؛ به‌ازای دسته صادقانه «سنجیده نشد» است. اصلاحِ داشبورد قراردادِ پین‌شده را عوض می‌کند ⇒ تصمیمِ کاربر |
