...
* بازگشت به نسخه‌ی قبلیِ کد **بدون بازگرداندنِ پشتیبان ممکن است**: جدول‌های
  canonical فقط بی‌استفاده می‌مانند و کدِ قدیمی روی همان فایل سالم کار می‌کند
  (`tests/test_canonical_schema.py` همین را پین می‌کند). بازگردانیِ پشتیبان
  اختیاری است — مگر بخواهید جدول‌های canonical را هم به عقب برگردانید. کلیدِ
  فرارِ فوری: `MKT_CANONICAL_ENABLE=0`. جزئیات در `ROLLBACK.md`.
